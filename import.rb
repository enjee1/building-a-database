...
zoning_types.each do |type|
  db_connection do |conn|
    result = conn.exec_params(
      "SELECT name FROM zoning_types WHERE name=$1", [type]
    )
...
construction_types.each do |type|
  db_connection do |conn|
    result = conn.exec_params(
      "SELECT name FROM construction_types WHERE name=$1", [type]
    )
...
db_connection do |conn|
  csv_records.each do |record|
    ...
    conn.exec_params("INSERT INTO accounts (name, construction_type_id, zoning_type_id)
      VALUES ($1, $2, $3)", [name, construction_type_id, zoning_type_id]
    )
  end
end
